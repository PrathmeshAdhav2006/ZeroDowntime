...
export default function Home() {
  ...
  return <main>
    <nav><div className="brand"><span className="brand-mark"><Icon name="shield" /></span><span>relay<span className="dot">.</span></span></div><div className="nav-right"><span className="live"><i /> LIVE ENVIRONMENT</span><span className="divider" /><span className="avatar">TS</span><span className="user">Tushar Sharma <small>⌄</small></span></div></nav>
    ...
    <section className="toolbar"><div><span className="service-dot" /><b>checkout-api</b><span className="tag">PRODUCTION</span></div><div className="toolbar-actions"><a className="ghost" href="/api/metrics" target="_blank" rel="noreferrer"><Icon name="chart" /> View metrics</a><button className="primary" onClick={deploy} disabled={state === "deploying" || state === "rolling-back"}><Icon name="bolt" /> Deploy v2.4.0</button></div></section>
